...
#Compared to perceiverCLS_FL, this one will add a 1d, 1x1 convolution before the perceiver network
#this is to serve as a channel expansion module so that perceiver embedding can be decoupled from the encoder embedding size
...
class LatentTransformer(nn.Module):
    ...
    This is different from v3. Will add concat option. 
    """
...
class PerceiverPositionalEncoding1D(nn.Module):
    ...
    >>> enc = PerceiverPositionalEncoding1D(512)
    >>> x = enc(x)
    """

    def __init__(self, d_model, max_len):
        super(PerceiverPositionalEncoding1D, self).__init__()
...
class PerceiverCLS(nn.Module):
    ...
    def __init__(
        ...
        #TO DO: Change this to the sepformer positional embedding or the mossformer one.
        self.embed = PerceiverPositionalEncoding1D(d_model = embed_dim, max_len = max_len)
        ...
        self.perceiver_blocks = nn.ModuleList([
            PerceiverBlock(
                ...
                attn_mlp_dim=attn_mlp_dim, #typical transformer MLP bottleneck dim, for the perceiver encoder
    ...
    def forward(self, x):
        ...
        #Next, we permute the input x because for the perceiver attention, x need to be [time, batch, filters]
        x = x.permute(2,0,1)
